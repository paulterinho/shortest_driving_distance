...
CITY = "Calgary"
DISTANCE_COL = 11 # each shape file will have a different number of columns depending on it's properties
SPATIAL_UNIT_ID = "CTUID" # will be different for every shape file


#download official 2021 Serbian census circles
u <- "ftp://ftp.gov.ab.ca/env/gda/TBF/Alberta_CT2016.zip"
download.file(u, basename(u), mode="wb")
unzip("Alberta_CT2016.zip")

#load census circles
pk <- st_read(paste0(getwd(), "/", "Alberta_CT2016.shp"), stringsAsFactors = FALSE) %>% 
        st_transform(4326) %>% 
        st_as_sf()

# define Belgrade's bounding box based on ggmap's bounding box
bg_map <- ggmap::get_map(getbb(CITY), 
	maptype = "toner-lite", 
	source = "stamen", 
	color="bw", 
	force=T)
...
# Try and rename the newly calculated distance column
names(b)[DISTANCE_COL] <- "dist"
...
geom_sf(data=bb, aes(fill = cat), color=NA, size=0) +
  ...
  labs(x = "©2021 Paul Walter\n Data: OSM Geofabrik",
    title = paste( "Shortest driving distance to a petrol station in ", CITY), 
